...
if (typeof gout == undefined) {
    ...
    xab = new AxisBuilder.ClockTime(graph, 0.0, 0.0, runningTime, true,
				"Time (Hours)");
...
var waitRV = new GaussianRV(MKS.minutes(15.0), MKS.minutes(3.0));
waitRV.setMinimum(MKS.minutes(2.0), true);
...
var tripEnds = 0;
var tripFails = 0;
var midstreamFails = 0;

var tl = new TripDataAdapter(scripting, {
    tripStarted: function(tripID, time, ticks, hub, domain) {
	tripStarts++;
    },
    tripPauseStart: function (tripID, time, ticks, hub) {
	tripEnds++;
    },

    tripPauseEnd: function(tripID, time, ticks, hub, domain) {
	tripStarts++;
    },
    ...
    tripFailedMidstream: function(tripID, time, ticks, hub) {
	tripFails++;
	midstreamFails++;
    }
});
...
output.println("number of midstream failures = " + midstreamFails);
