...
// create a storage hub and configure how its workers will
// be used.

var shub = shf.createObject("shub",[
    {
	"x": MKS.miles(0.25),
	"y": 0.0,
	"sysDomain": sysDomain,
	"nworkersNoPickup": 0,
	"nworkersWithPickup": 1,
	"intervalWithPickup": MKS.hours(1.1),
	"nworkersFixOnly": 0},
    {withPrefix: "hubTable",
     withIndex: [
	 {"hub": hub1},
...
var hwl = new HubWorkerAdapter(scripting, {
    dequeued: function(worker, time, ticks, hub) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d),"
		       + "hub \"%s\":: dequeued\n",
		       worker.getName(), time, ticks, hub.getName());
	foutput.flush();
    },

    enteredHub: function(worker, time, ticks, hub) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d): entered hub \"%s\"\n",
		       worker.getName(), time, ticks, hub.getName());
	foutput.flush();
    },

    fixingOverflows: function(worker, time, ticks, hub) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d), hub \"%s\": %s\n",
		       worker.getName(), time, ticks, hub.getName(),
		      "fixing oveflows");
	foutput.flush();
    },

    fixingPreferred: function(worker, time, ticks, hub) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d), hub \"%s\": %s\n",
		       worker.getName(), time, ticks, hub.getName(),
		      "fixing preferred");
	foutput.flush();
    ...
    leftHub: function(worker, time, ticks, hub) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d): left hub \"%s\"\n",
		       worker.getName(), time, ticks, hub.getName());
	foutput.flush();
    },

    queued: function(worker, time, ticks, hub) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d), hub \"%s\": queued\n",
		       worker.getName(), time, ticks, hub.getName());
	foutput.flush();
    },

    changedCount:function(worker, time, ticks, hub, oc, nc) {
	foutput.format("worker \"%s\" at t=%g (ticks=%d), hub \"%s\": "
		       + " bicycle count changed from %d to %d\n",
		       worker.getName(), time, ticks, hub.getName(), oc, nc);
	foutput.flush();
    }
});

worker1.addHubWorkerListener(hwl);

// Run the simulation for a default of 120  minutes of simulation time.
// The actual time is provided by rtime, which can be modified at the
// command line
